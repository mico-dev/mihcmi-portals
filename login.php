...
//presubmit
    ...
    if(isset($_POST['submit'])) {
        ...
        //new 05-26-21
        #$sql = mysqli_query($con, "SELECT rt_id, rt_user, rt_pass FROM `tb_rtpcrnewcif` WHERE BINARY rt_user='$uname' AND rt_pass='$pword' UNION ALL SELECT rt_id, rt_user, rt_pass FROM `tb_rtpcrnewv9` WHERE BINARY rt_user='$uname' AND rt_pass='$pword'") or die(mysqli_error($con));
        $sql = mysqli_query($con, "SELECT rt_id, rt_user, rt_pass FROM `tb_rtpcrnewv9` WHERE BINARY rt_user='$uname' AND rt_pass='$pword'") or die(mysqli_error($con));
        ...
        if($countcheck <> 0 ) {
            ...
            while($row = mysqli_fetch_array($checkapp)) {
               ...
               $_SESSION['ssid'] = $ru_password;
                ...
                //redirect if success
                header("Location: rtapp-profile.php");
                end();
