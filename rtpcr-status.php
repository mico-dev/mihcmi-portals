...
/*$sql = mysqli_query($con, "SELECT rt_id, rt_timestamp, rt_linestatus, rt_validation, rt_dateofinterview, rt_validationts, rt_sentdate, rt_lname, rt_fname, rt_mname, rt_suffix FROM tb_rtpcrnewv9 WHERE rt_id='$id'") or die(mysqli_error($con));
...
$sql = $con->query("SELECT rt_id, rt_timestamp, rt_linestatus, rt_validation, rt_dateofinterview, rt_validationts, rt_sentdate, rt_lname, rt_fname, rt_mname, rt_suffix, rt_sendstatus, rt_si1collected, rt_si1collectedtime, rt_ctrtimestamp, rt_vipstatustime  FROM tb_rtpcrnewv9 WHERE rt_id='$id'") or die($con->error);
...
$a = date('m-d-y H:i:s A', strtotime($rt_timestamp));
$b = date('m-d-y H:i:s A', strtotime($noon));
$twopm = date('m-d-y H:i:s A', strtotime($pm));
$nineam = date('m-d-y 09:00:00 A', strtotime($timestamp));
if($a<=$b) {
	#$received = $noon;
	$received =  " 10:00 AM";
}elseif(($a>$nineam)AND($a<$twopm)){
	$received =  " 15:00 PM";
} else {
	$receivedquery = mysqli_query($con, "SELECT rt_id, rt_dateofinterview, rt_timestamp FROM `tb_rtpcrnewcif` WHERE rt_dateofinterview='$dateofinterview' UNION ALL SELECT rt_id, rt_dateofinterview, rt_timestamp FROM `tb_rtpcrnewv9` WHERE rt_dateofinterview='$dateofinterview' ORDER BY rt_id DESC LIMIT 1")or die(mysqli_error($con));
	while($row=mysqli_fetch_assoc($receivedquery)) {
		$timestamp = $row['rt_timestamp'];
